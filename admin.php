<?php
class web_robotstxt_admin {

	function web_robotstxt_admin() {
		// stuff to do when the plugin is loaded
		add_action( 'admin_menu', array( &$this, 'admin_menu' ) );
	}

	function admin_menu() {
		add_options_page( 'Ultimate Robots.txt Settings', 'Ultimate Robots.txt', 'manage_options', __FILE__, array( &$this, 'settings_page' ) );
	}// end function

	function settings_page() {
		
		global $web_robotstxt;
		
		$options = $web_robotstxt->get_options();
		...
		echo '<div class="wrap">'
			.'<h2>Ultimate Robots.txt Settings</h2>'
			.'<form method="post">';
		if ( function_exists( 'wp_nonce_field' ) ) wp_nonce_field( 'web_robotstxt_settings' );
...
$web_robotstxt_admin = new web_robotstxt_admin;
